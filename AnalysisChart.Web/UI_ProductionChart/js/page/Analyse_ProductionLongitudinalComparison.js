...
function LoadEquipmentCommonInfo() {
    $.ajax({
        type: "POST",
        url: "Analyse_ProductionLongitudinalComparison.aspx/GetEquipmentInfo",
        data: "",
...
function LoadIndicatorItems() {
    $.ajax({
        type: "POST",
        url: "Analyse_ProductionLongitudinalComparison.aspx/GetIndicatorItems",
        data: "",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (msg) {
...
function LoadComparableIndexData(myValueType, myCategory, myLoadType) {                                      //装载可比数据
    ...
    if (myCategory == 'StandardItems' || myCategory == AllCategroy) {
        ...
        $.ajax({
            type: "POST",
            url: "Analyse_ProductionLongitudinalComparison.aspx/GetStandardItems",
            data: "{myStatisticalMethod:'" + 'Entity' + "',myValueType:'" + myValueType + "'}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (msg) {
    ...
    if (myCategory == 'CustomDefine' || myCategory == AllCategroy) {
        ...
        $.ajax({
            type: "POST",
            url: "Analyse_ProductionLongitudinalComparison.aspx/GetCustomDefineTagsGroup",
            data: "",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (msg) {
    ...
    $('#' + myGridId).tree({
        ...
        onDblClick: function (rowData) {
            if (rowData.id.length > 3) {
                ...
                if (m_SameTimeOfLastCyc == TimeOfLastCycName) {
                    var m_NewRow = {
                        'TagItemId': m_TagItemId, 'TagItemName': m_TagName,
                        'TagId': m_VariableId, 'TagStaticsType': "2",
                        'TagTable': rowData.OrganizationId, 'TagDataBase': '', 'TagDescription': "统计数据",
                        'TagTabClass': "EntityProductionStatics", 'SameTimeOfLastCyc': "", 'OtherInfo': ""
                    };
                    AddChartTags(m_NewRow);
                }
                var m_NewRow = {
                    ...
                    'TagId': m_VariableId, 'TagStaticsType': "2",
                    'TagTable': rowData.OrganizationId, 'TagDataBase': '', 'TagDescription': "统计数据",
                    'TagTabClass': "EntityProductionStatics", 'SameTimeOfLastCyc': m_SameTimeOfLastCyc, 'OtherInfo': ""
                };
                var m_AddFlag = AddChartTags(m_NewRow);
                if (m_AddFlag == true) {
                    alert("标签添加成功!");
...
function SetSelectGridDataGroup(myTagGroupId) {
    $.ajax({
        type: "POST",
        url: "Analyse_ProductionLongitudinalComparison.aspx/GetTagGroupJsonById",
...
function SaveDataTagsGroupFun() {
    ...
    if (m_Valid_TagsGroupName == false) {
        ...
        $.ajax({
            type: "POST",
            url: "Analyse_ProductionLongitudinalComparison.aspx/SaveCustomDefineTags",
...
function RemoveAllDataTagGroupFun() {
    parent.$.messager.confirm('询问', '您确定要删除所有自定义组?', function (r) {
        if (r) {
            $.ajax({
                type: "POST",
                url: "Analyse_ProductionLongitudinalComparison.aspx/DeleteAllCustomDefineTagsByUserId",
                data: "",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (msg) {
...
function RemoveDataTagGroupById(myId) {
    parent.$.messager.confirm('询问', '您确定要删除该自定义组?', function (r) {
        if (r) {
            $.ajax({
                type: "POST",
                url: "Analyse_ProductionLongitudinalComparison.aspx/DeleteCustomDefineTagsByTagsGroupId",
...
function ExportFileFun() {
    ...
    form.attr('action', "Analyse_ProductionLongitudinalComparison.aspx");
...
function LoadLinesDataFun() {
    ...
    if (m_StartTime == "") {
        ...
        if (m_TagInfoObject['rows'].length > 0) {
            ...
            $.ajax({
                type: "POST",
                url: "Analyse_ProductionLongitudinalComparison.aspx/GetChartDataJson",
