...
function LoadStandardItems() {
    var m_ValueTypeCombobox = $('#ComboTree_ValueTypeF').combotree('getValue');
    ...
    $('#' + myGridId).tree({
        ...
        onDblClick: function (rowData) {
            if (rowData.id.length > 5) {
                ...
                var m_ValueTypeName = $('#ComboTree_ValueTypeF').combotree('getText');

                var m_ValueType = $('#ComboTree_ValueTypeF').combotree('getValue');
                var m_TagItemId = m_ValueType;

                var m_ValueTypeCombobox = $('#ComboTree_ValueTypeF').combotree('getValue').split('_');
                var m_StatisticalMethod = "Entity";
...
function AddChartTags(myRow) {
    ...
    if (m_TagsCount < 32) {
        for (var i = 0; i < m_TagsCount; i++) {
            if (m_Rows[i]["VariableId"] == myRow["VariableId"] && m_Rows[i]["TagItemId"] == myRow["TagItemId"] && m_Rows[i]["OrganizationId"] == myRow["OrganizationId"]) {
                m_IsDuplicate = true;
                break;
            }
        }
...
function LoadChartFun() {
    ...
    if (m_TagInfoObject != undefined && m_TagInfoObject != null) {
        for (var i = 0; i < m_TagInfoObject.rows.length; i++) {
            m_TagInfoObject.rows[i].StatisticName = m_ValueTypeName;
            m_TagInfoObject.rows[i].TagItemId = m_ValueType;
        }
    }
    SelectDatetime = "开始时间:" + m_StartTime + "--结束时间" + m_EndTime + "(" + $('#ComboTree_ValueTypeF').combotree('getText') + ")";
