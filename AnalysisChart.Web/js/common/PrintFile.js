...
{
    var m_Columns = $('#' + myDataGridId).datagrid("getColumnFields", false);
    var m_Rows = $('#' + myDataGridId).datagrid("getRows", false);

    var m_DataTable = $('#' + myDataGridId).datagrid("getPanel").html();
    ...
    //////////////////////////字段名////////////////////
    for (var i = 0; i < m_DataGridFrozenColumnsObj.length; i++) {
        for (var j = 0; j < m_DataGridFrozenColumnsObj[i].length; j++) {
            if (m_DataGridFrozenColumnsObj[i][j]["hidden"] != true) {    //只导出显示出来的数据
                ...
                if (m_ColSpan > 1) {
                    m_CellItem = + ' colspan=' + m_ColSpan;
                }
                m_CellItem = ' style = "border:0.1pt solid black; text-align:center;">';
                m_ColumnNamesHtml = m_ColumnNamesHtml + m_CellItem + m_DataGridFrozenColumnsObj[i][j]["title"] + '</td>';
            }
        }
    }
    for (var i = 0; i < m_DataGridColumnsObj.length; i++) {
    ...
    for (var i = 0; i < m_DataGridDataObj["rows"].length; i++) {
        m_ColumnNamesHtml = m_ColumnNamesHtml + "<tr>"
        $.each(m_DataGridDataObj["rows"][0], function (i, value) {
            

            //this;      //this指向当前属性的值

            //i;      //name表示Object当前属性的名称

            var aa= value;     //value表示Object当前属性的值
